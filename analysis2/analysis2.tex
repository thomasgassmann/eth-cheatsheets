...
Die Menge $S$ an Lösungen ist eine Untermenge des Raums der komplexen Funktionen auf $I$ mit Dimension $n$.\\
...
\[x^l e^{\beta_j x} \Big( \cos(\gamma_j x) + i \sin(\gamma_j x) \Big) \quad \text{für }0 \leq l < m_j\]
...
Der Gradient an einem Punkt $x_0$ ist immer orthogonal zum Tangentenvektor $\gamma'(t_0)$ (für $\gamma(t_0) = x_0$) wobei $\gamma$ eine Kurve auf dem Level set ist. 
