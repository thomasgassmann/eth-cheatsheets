...
Dieses Cheatsheet basiert auf dem \href{https://github.com/XYQuadrat/eth-cheatsheets}{Analysis II Cheatsheet von xyquadrat}.
...
$$f: \R^2 \mapsto \R, (x, y) \mapsto \frac{x^2y}{x^2 + y^2}$$

$f$ hat alle partiellen Ableitungen, ist stetig in $(0, 0)$ aber nicht differenzierbar.
...
Allgemein: ist $f$ in $C^k$, dann kommutieren alle Ableitungen bis zur Ordnung $k$. Falls $f \notin C^k$ ist bspw. $f(x,y) = \frac{xy(x^2 - y^2)}{x^2 + y^2}$ mit $f(0, 0) = 0$ ein Gegenbeispiel.
...
Allgemeiner können wir für jede Funktion $g$ ein Vektorfeld $f = (f_1, f_2)$ finden so dass $g = \partial_x f_2 - \partial_y f_1$. Bspw. $f_1 = 0$ und $f_2$ so dass $\partial_x f_2 = g$.
