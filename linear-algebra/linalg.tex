...
Eine $n \times n$ Matrix heisst \textbf{unitär} falls $A^H A = I_n$. Eine Matrix heisst \textbf{orthogonal} falls $A^T A = I_n$. Es gilt $\det(A) = \pm 1$. Die Matrix ist genau dann unitär/orthogonal wenn alle Spalten orthonormal sind.
...
\begin{subbox}{Eigenschaften der Determinante}
  ...
  \begin{itemize}
    ...
    \item Vertauscht man zwei Zeilen (oder Spalten), dann wechselt $\det(A)$ das Vorzeichen
    ...
    \item $\det(I) = 1$
    \item $\det(A^H) = \overbar{\det(A)}$ sowie $\det(A^T) = \det(A)$
