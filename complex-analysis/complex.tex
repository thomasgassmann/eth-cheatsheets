...
\begin{subbox}{Homotopie}
  ...
  \begin{equation*}
    ...
    \end{matrix} ~~~ \forall t \in [a,b] ~~~ \text{ und } ~~~ \begin{matrix} H(s,a)=\alpha\\ H(s,b) = \beta \end{matrix} ~~~ \forall s\in [0,1] \end{equation*} gibt. Die Funktion \(H\) ist die sogenannte Homotopie von \(\gamma_0\) und \(\gamma_1\).
...
\begin{center}
 ...
 \begin{center}
  \begin{tabularx}{\linewidth}{>{\centering\arraybackslash}X>{\centering\arraybackslash}X}
   ...
   $\int x \cos(ax) \dx$ & $\frac1a x \sin\left(a x\right) + \frac{1}{a^2} \cos\left(a x\right)$ \\
   \bottomrule
  \end{tabularx} 
 \end{center}
...
Wir verwenden hier die übliche Definition der Fourier-Transformation mit $\frac{1}{\sqrt{2\pi}}$.
...
\begin{center}
  ...
  $t \cosh(at)$ & $\frac{s^2 + a^2}{(s^2 - a^2)^2}, \; \Re(s) >  a$\\
  $t \sinh(at)$ & $\frac{2as}{(s^2 - a^2)^2}, \; \Re(s) >  a$\\
  $\delta(t - a)$ & $e^{-as}$\\
  \bottomrule
  \end{tabularx}
\end{center}
